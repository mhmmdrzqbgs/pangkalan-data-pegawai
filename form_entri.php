...
<div class="bg-white rounded-4 shadow-sm p-4 mb-5">
    ...
    <div class="alert alert-primary rounded-4 mb-5" role="alert">
        <i class="fa-solid fa-pen-to-square me-2"></i> Entri Data Pegawai
    </div>
    <!-- form entri data -->
        ...
        <div class="row">
            <div class="col-xl-6">
                <div class="row g-0">
                    <div class="mb-3 col-xl-6 pe-xl-3">
                        ...
                        $query = $mysqli->query("SELECT RIGHT(id_pegawai,5) as nomor FROM pegawai ORDER BY id_pegawai DESC LIMIT 1")
                                                or die('Ada kesalahan pada query tampil data : ' . $mysqli->error);
                        // ambil jumlah baris data hasil query
                        $rows = $query->num_rows;

                        // cek hasil query
                        // jika "id_pegawai" sudah ada
                        if ($rows <> 0) {
                            // ambil data hasil query
                            $data = $query->fetch_assoc();
                            // nomor urut "id_pegawai" yang terakhir + 1
                            $nomor_urut = $data['nomor'] + 1;
                        }
                        // jika "id_pegawai" belum ada
                        else {
                            // nomor urut "id_pegawai" = 1
                            $nomor_urut = 1;
                        }

                        // menambahkan karakter "ID-" diawal dan karakter "0" disebelah kiri nomor urut
                        $id_pegawai = "ID-" . str_pad($nomor_urut, 5, "0", STR_PAD_LEFT);
                        ?>
                        <label class="form-label">ID Pegawai <span class="text-danger">*</span></label>
                        <!-- tampilkan "id_pegawai" -->
                        <input type="text" name="id_pegawai" class="form-control" value="<?php echo $id_pegawai; ?>" readonly>
                    </div>

                    <div class="mb-3 col-xl-6 pe-xl-3">
                        <label class="form-label">Tanggal Masuk <span class="text-danger">*</span></label>
                        <input type="text" name="tanggal_masuk" class="form-control datepicker" autocomplete="off" required>
            ...
            <div class="col-xl-6">
                <div class="mb-3 ps-xl-3">
                    ...
                    <select name="divisi" class="form-select" autocomplete="off" required>
